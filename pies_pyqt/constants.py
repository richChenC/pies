...
SOFTWARE_NOTES = [
    ...
    "探头寿命以累计检测管道数量为主，时间信息仅用于记录过程与异常核对。",
    "开始时间和结束时间优先取同组标记文件的实际时间；缺失关键标记时只做异常提示，不伪造时间。",
    "管道数量只统计真实检测数据，不将 999、888 等标记性文件计入真实管道数量；表格中的累计管道数量会持续累加。",
...
SAFETY_REMINDERS = [
    ...
    "导入网络路径时，请确保当前账号至少具备读取权限，并避免在源目录中直接手工编辑数据。",
